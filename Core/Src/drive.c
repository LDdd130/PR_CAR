...
#define CENTER_BASE_SPEED_PCT      32.0f   /* Straight driving speed */
#define CENTER_MIN_SPEED_PCT       24.0f   /* Speed at max steering command */
...
static void Drive_CenteringPD_Run(const DriveInputs *in)
{
    ...
    else
    {
        ...
        steer = clampf(steer, -CENTER_STEER_MAX_PCT, CENTER_STEER_MAX_PCT);
        prev_error = error;
    }
    ...
    float lt = clampf(base - steer, 0.0f, 100.0f);
