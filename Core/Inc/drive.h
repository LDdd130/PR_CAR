...
#define ARC_OUTER        50   /* 코너 아크 바깥 바퀴 전진 [%duty]. 고정 저속이 아니라 진행각 램프로 출구만 감속한다 →
                                 우선 충돌 0을 위해 속도/횡가속을 낮춘다
                                 (testtrack 코너 8개 × ~1s → 20s 목표의 코너 몫. 아크 중단선은 CORNER_ABORT_CM이 별도 방어) */
#define ARC_INNER        21   /* 코너 아크 안쪽 바퀴 전진 [%duty]. 반경은 유지하고 평균속도만 소폭 회복 */
#define ARC_APPROACH_OUTER 36 /* 목표각 근접 시 감속 아크 바깥 바퀴 */
#define ARC_APPROACH_INNER 16 /* 목표각 근접 시 감속 아크 안쪽 바퀴 */
#define ARC_APPROACH_DEG 34.0f /* 목표각까지 이 각도 이내부터 선형 감속 램프 */
...
#define FRONT_ARC_CM    52   /* 정면 이 거리 미만 + 한쪽 측면 트임/비대칭 = 전진성 아크 코너 진입.
                                IMG_2985 기준 전방이 충분히 열려 있으면 straight-lock을 유지하고, 코너 직전에서만 ToF 코너 판단을 허용 */
...
#define CORNER_NEAR_SAFE_CM 10 /* 비대칭 코너 인정 최소 근접측 여유. 37cm 최협/차폭 16cm 중앙 여유≈10.5cm 기준 */
...
#define KP_HDG          0.58f/* heading-hold P게인 [%duty/deg]. IMG_2985 straight-lock에서 진행축 복귀 권한을 키운다 */
...
#define SIDE_AVOID_CM       9   /* 진입: 한쪽 측면이 이 값 미만 = 벽 스침 위험 → 비상 회피.
                                   최협 37cm 중앙여유 10.5cm보다 낮아 정상 중앙주행 오발동 없음.
                                   IMG_2982는 8cm 진입이 늦어 벽을 평행하게 긁는 패턴이라 9cm로 선제화 */
#define SIDE_AVOID_CLEAR_CM 10  /* 탈출: 최협 중앙 여유 근처까지 회복되면 복귀. 37cm 병목에서도 달성 가능해야 한다 */
...
#define CORNER_GRID_EXIT_CM      52   /* 45° 코너 조기탈출 전방 트임. 90° 정션 중간(45° 대각)에서 외벽 빔은
                                         37~67cm 정션 기준 ~26..47cm라 이 선을 못 넘고, 진짜 45° 출구는
                                         새 레그 축방향 정반사로 이 이상이 나온다 (FRONT_ARC_CM와 동일값) */
