...
/**
  ******************************************************************************
  * @file    ultra.h
  * @brief   HC-SR04 '전방 1채널' 초음파 측정 모듈 (TIM3 CH1 입력캡처)
  *          - front: TRIG PA5 / ECHO TIM3_CH1(PA6). 측면은 VL53L0X ToF로 이관(vl53l0x.h)
  *          - RTOS 전용: 에코 대기 = osThreadFlagsWait. TIM3 IC ISR이 '유효 측정 완료' 시
  *            waiter 태스크로 flag set — TIM3 prio 5 = configMAX_SYSCALL 경계라 합법.
  *            무에코/글리치는 wait_ms timeout으로 끝남 (구 busy-wait와 동일 의미).
  *          - Ultra_Init은 SensorTask 기동 시 1회 (IC Start까지 여기서 — pre-kernel ISR 창 제거)
  *          - 측정 상한/노브(ULTRA_MAX_CM, MEAS_WAIT_MS)는 drive.h 단일 블록 유지
  ******************************************************************************
  */
...
/* waiter(측정 호출 태스크) 등록 + TIM3 IC CH1 start. 루프 진입 전 1회 호출 */
void Ultra_Init(osThreadId_t waiter);

/* 전방 센서 측정 (블로킹: 에코 회신 즉시 wake, 무에코/글리치만 wait_ms 상한).
...
uint16_t median_n(const uint16_t *a, uint8_t n);  /* 중앙값: 스파이크 제거 (조향 안정성). n 홀수 권장 */
